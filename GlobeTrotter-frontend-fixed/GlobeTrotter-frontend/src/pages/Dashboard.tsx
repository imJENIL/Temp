...
export function Dashboard() {
  ...
  return <div className="space-y-8 pb-8">
    ...
    <section className="panel overflow-hidden p-6 sm:p-8">
      {dashboardTrips.length === 0 ? <div className="grid min-h-[280px] place-items-center text-center"><div className="max-w-lg"><Pill tone="green">Your dashboard</Pill><h2 className="mt-4 font-display text-3xl font-extrabold sm:text-4xl">Your next adventure starts here</h2><p className="mx-auto mt-3 max-w-md text-black/50">Create a trip to see it appear here instantly. Europe Adventure remains available in My Trips.</p><Link to="/trips/new" className="btn-primary mt-6"><Plus size={17}/> Create your first trip</Link></div></div> : <div><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><Pill tone="green">Latest planning activity</Pill><h2 className="mt-3 font-display text-3xl font-extrabold">Your trips</h2><p className="mt-1 text-sm text-black/45">New trips appear here immediately after creation.</p></div><Link to="/trips" className="text-sm font-bold text-mint">View all trips</Link></div><div className="mt-6 grid gap-5 xl:grid-cols-2">{dashboardTrips.map((trip) => <TripCard key={trip.id} trip={trip}/>)}</div></div>}
    </section>
